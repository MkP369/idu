...
static EMH_INLINE void wymum(uint64_t *A, uint64_t *B) {
  ...
  *A = _umul128(*A, *B, B);
  ...
  uint64_t ha = *A >> 32, hb = *B >> 32, la = static_cast<uint32_t>(*A),
           lb = static_cast<uint32_t>(*B);
...
static const uint64_t emh_wyp[4] = {
...
// Marked EMH_INLINE: always inlined into hash_key() hot path — no function call
// overhead.
static EMH_INLINE uint64_t emh_wyhash(const void *key, size_t len,
                                      uint64_t seed) {
...
// This allows existing code that calls wyhash(key, len, seed) to work without
// external wyhash.h. If an external wyhash.h is already included
// (wyhash_final_version_4_2 is defined), skip the alias to avoid conflicts.
...
static EMH_INLINE uint64_t emh_wyhash64(uint64_t A, uint64_t B) {
  ...
  emhash_detail::wymum(&A, &B);
  return emhash_detail::wymix(A ^ 0x2d358dccaa6c78a5ull,
                              B ^ 0x8bb84b93962eacc9ull);
}
} // namespace emhash_detail
